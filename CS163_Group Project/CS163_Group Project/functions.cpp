...
void VE::insertWord(VTrie*& root, wstring& word, wstring& definition)//UY
{
	word = VToLower(word);
	VTrie* current = root;
	for (wchar_t& c : word)
...
void VE::insertWord(VTrie*& root, wstring& word, vector<wstring> &definition,wstring example)//UY
{
	word = VToLower(word);
	VTrie* current = root;
	for (wchar_t& c : word)
...
bool VE::loadRawData(VTrie*& root, string path)//UY
{
	...
	else
	{
		...
		while (getline(fin, word, L'|'))
		{
			definitions.clear();
			wstring numMeaning;
			getline(fin, numMeaning, L'|');
			int num = stoi(numMeaning);
			for (int i = 0; i < num; i++)
			{
				wstring def;
				getline(fin, def, L'|');
			...
			getline(fin, example, L'\n');
			VE::insertWord(root, word, definitions,example);
		}
		return true;
	}
}
